...
func (ic *IPPoolController) syncHandler(ctx context.Context, pool *spiderpoolv1.SpiderIPPool) error {
	if pool.DeletionTimestamp != nil {
		...
		autoPoolDesiredIPNumberStr, ok := pool.GetLabels()[constant.LabelAutoPoolDesiredIPNumber]
		if ok {
			autoPoolDesiredIPNumber, err := strconv.Atoi(autoPoolDesiredIPNumberStr)
			if nil != err {
				return fmt.Errorf("%w: failed to parse auto-created IPPool %s label AutoPoolFirstIPNumber: %v", constant.ErrWrongInput, pool.Name, err)
			}
			if autoPoolDesiredIPNumber != 0 && len(pool.Spec.IPs) == 0 {
				// just create the empty IPPool, wait for a second
				return nil
			}
		}
