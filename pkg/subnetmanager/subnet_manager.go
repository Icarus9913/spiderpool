...
func NewSubnetManager(config SubnetManagerConfig, client client.Client, apiReader client.Reader, ipPoolManager ippoolmanager.IPPoolManager, reservedIPMgr reservedipmanager.ReservedIPManager) (SubnetManager, error) {
	if client == nil {
...
func (sm *subnetManager) ReconcileAutoIPPool(ctx context.Context, pool *spiderpoolv1.SpiderIPPool, subnetName string,
	...
	if operationCreate {
		...
		poolLabels := map[string]string{
			...
			constant.LabelAutoPoolDesiredIPNumber:   strconv.Itoa(autoPoolProperty.DesiredIPNumber),
		}
	...
	ips, err := sm.preAllocateIPsFromSubnet(ctx, subnet, pool, autoPoolProperty.IPVersion, autoPoolProperty.DesiredIPNumber)
	if nil != err {
		return nil, fmt.Errorf("failed to pre-allocate auto-create IPPool %s IPs from SpiderSubnet %s: %w", pool.Name, subnetName, err)
	}
	pool.Spec.IPs = ips
	labels := pool.GetLabels()
	labels[constant.LabelAutoPoolDesiredIPNumber] = strconv.Itoa(autoPoolProperty.DesiredIPNumber)
...
func (sm *subnetManager) preAllocateIPsFromSubnet(ctx context.Context, subnet *spiderpoolv1.SpiderSubnet, pool *spiderpoolv1.SpiderIPPool, ipVersion types.IPVersion, desiredIPNum int) ([]string, error) {
	log := logutils.FromContext(ctx)
	...
	if ok {
		...
		if desiredIPNum == len(subnetPoolIPs) {
			...
			if len(subnetPoolIPs)-len(poolIPAllocations) >= len(subnetPoolIPs)-desiredIPNum {
				...
				log.Sugar().Warnf("===========desiredIPNum '%d' =================IPPool '%v' ====================subnetPoolAllocation '%v'==========", desiredIPNum, pool, subnetPoolAllocation)

				// free IPs
				freeIPs := spiderpoolip.IPsDiffSet(subnetPoolIPs, poolAllocatedIPs, true)
				log.Sugar().Warnf("+++++++++++++++++++++++++freeIPs: '%v' ++++++++++++++len(subnetPoolIPs) '%d' +++++++++++++++++++desiredIPNum '%d' ++++++++++++++++++", freeIPs, len(subnetPoolIPs), desiredIPNum)
				...
				subnetControlledIPPools[pool.Name] = spiderpoolv1.PoolIPPreAllocation{IPs: poolIPRange}
			...
			log.Sugar().Debugf("IPPool %s incresed its desired IP number from %d to %d, and the last allocation is %v",
	...
	newlyControlledIPPools[pool.Name] = spiderpoolv1.PoolIPPreAllocation{IPs: allocateIPRange}
